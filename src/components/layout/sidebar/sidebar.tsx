...
export const Sidebar: React.FC<SidebarProps> = ({ isOpen, toggleSidebar }) => {
  ...
  // Sənin verdiyin yeni BineAgro Strukturu
  const menuItems: MenuItem[] = [
    ...
    {
      title: "Əsas Səhifə",
      icon: LayoutTemplate, // Website icon
      submenu: [
  ...
  const handleMenuClick = (title: string) => {
    if (!isOpen) toggleSidebar(); // Əgər sidebar bağlıdırsa, açsın
  ...
  className={cn(
    ...
    // Yalnız eni (width) animasiya olsun
...
>
            ...
            <div key={item.title}>
              ...
              <div
                ...
                className={cn(
                  ...
                  // Linkdirsə NavLink kimi davranacaq, Submenyu isə div kimi
                  !item.submenu ? "cursor-pointer" : "cursor-pointer",

                  // Aktivlik Rəngləri (Ana menyu və ya submenyu aktivdirsə)
              ...
              >
                {/* Linkməntiqi: Əgər URL varsa NavLink, yoxsa sadəcə görüntü */}
                {item.url ? (
                ...
                {isOpen && (
                  <>
                    <span className="flex-1 animate-in fade-in slide-in-from-left-2 duration-300 whitespace-nowrap overflow-hidden text-sm font-medium">
                      {item.title}
                    </span>
                    {/* Dropdown Oxu */}
                    {item.submenu && (
              ...
              {isOpen &&
                item.submenu &&
                (openMenu === item.title || isSubActive) && (
                  <div className="mt-1 ml-4 pl-4 border-l border-border space-y-1 animate-in slide-in-from-top-2 fade-in duration-300">
      ...
      <div className="p-4 mb-2 border-t border-border/5">
        ...
        >
          ...
          {isOpen && (
            <span className="font-medium whitespace-nowrap">Çıxış</span>
          )}
        </button>
      </div>
    </aside>
